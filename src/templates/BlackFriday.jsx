...
export function BlackFriday({ producto, precio, mensaje, logo, tamaño }) {
  return (
    <article className={`poster-page relative mx-auto w-full max-w-[794px] overflow-hidden bg-black text-white ${getTemplateSizeClass(tamaño)}`} style={getTemplateStyle(tamaño)}>
      ...
      <div className="absolute right-8 top-8 rounded-full border-4 border-yellow-300 px-6 py-3 text-lg font-black uppercase text-yellow-300">{logo || 'Mega sale'}</div>
      <div className="relative flex h-full flex-col items-center justify-center px-[8%] pt-[15%] text-center">
        <p className="rotate-[-2deg] bg-yellow-300 px-9 py-4 text-3xl font-black uppercase tracking-wider text-black shadow-[8px_8px_0_#dc2626]">{mensaje || 'Oferta explosiva'}</p>
        <div className="mt-10 leading-none text-yellow-300 [text-shadow:7px_7px_0_#dc2626]">
        ...
        <h2 className="mt-10 max-w-[90%] break-words font-black uppercase leading-[0.9]" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
      </div>
    </article>
  );
}
