      ...
      <div className="flex h-full flex-col justify-between px-[8%] py-[10%] text-gray-950">
        <header className="flex items-start justify-between gap-6 border-b-4 border-gray-950 pb-8">
          <div>
            ...
            <h1 data-editable-key="message" className="mt-3 text-5xl font-black uppercase leading-none">{mensaje || 'Precio por volumen'}</h1>
          </div>
          <p data-editable-key="logo" className="rounded-full border-4 border-gray-950 px-6 py-3 text-lg font-black uppercase">{logo || 'Almacén'}</p>
        </header>
        <section className="text-center">
          <div data-editable-key="price" className="leading-none text-gray-950">
          ...
          <h2 data-editable-key="product" className="mx-auto mt-10 max-w-[92%] break-words font-black uppercase leading-[0.9]" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
        </section>
