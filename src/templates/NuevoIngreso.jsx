  ...
  return (
    <article className={`poster-page relative mx-auto w-full max-w-[794px] overflow-hidden bg-sky-600 text-white ${getTemplateSizeClass(tamaño)}`} style={getTemplateStyle(tamaño)}>
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-[8%] pt-[16%] text-center">
        <p className="rounded-2xl bg-sky-950/80 px-8 py-4 text-2xl font-black uppercase tracking-wider">{mensaje || 'Nuevo ingreso'}</p>
        <div className="mt-9 leading-none text-yellow-200 [text-shadow:5px_5px_0_#075985]">
        ...
        <h2 className="mt-8 max-w-[92%] break-words font-black uppercase leading-[0.92]" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
        <p className="mt-8 text-xl font-black uppercase tracking-[0.3em] text-sky-100">{logo || 'Probalo hoy'}</p>
      </div>
    </article>
  );
}
