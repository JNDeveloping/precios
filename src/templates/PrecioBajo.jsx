  ...
  return (
    <article className={`poster-page relative mx-auto w-full max-w-[794px] overflow-hidden bg-emerald-600 text-white ${getTemplateSizeClass(tamaño)}`} style={getTemplateStyle(tamaño)}>
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-[8%] text-center">
        ...
        <p data-editable-key="message" className="rounded-full bg-white px-8 py-3 text-2xl font-black uppercase tracking-wide text-emerald-700">{mensaje || 'Precio bajo todos los días'}</p>
        <div data-editable-key="price" className="mt-8 leading-none text-white [text-shadow:6px_6px_0_#064e3b]">
        ...
        <h2 data-editable-key="product" className="mt-8 max-w-[90%] break-words font-black uppercase leading-[0.92]" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
        <p data-editable-key="logo" className="absolute bottom-10 text-lg font-black uppercase tracking-[0.35em] text-emerald-100">{logo || 'Almacén ahorro'}</p>
      </div>
    </article>
  );
}
