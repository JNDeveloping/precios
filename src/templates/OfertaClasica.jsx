...
export function OfertaClasica({ producto, precio, mensaje, logo, tamaño }) {
  return (
    <article className={`poster-page relative mx-auto w-full max-w-[794px] overflow-hidden bg-white ${getTemplateSizeClass(tamaño)}`} style={getTemplateStyle(tamaño)}>
      ...
      <div className="absolute right-8 top-8 rounded-full border-4 border-red-600 bg-white px-5 py-3 text-lg font-black uppercase text-red-600 shadow-xl">{logo || 'MARKET'}</div>
      <div className="relative flex h-full flex-col items-center justify-center px-[7%] pb-[12%] pt-[20%] text-center">
        <p className="rounded-full bg-yellow-300 px-8 py-3 text-2xl font-black uppercase tracking-wide text-red-700 shadow-lg">{mensaje || '🔥 IMPERDIBLE'}</p>
        <div className="mt-8 max-w-full leading-none tracking-tighter text-red-600">
        ...
        <h2 className="mt-8 max-w-[92%] text-balance break-words font-black uppercase leading-[0.95] tracking-tight text-gray-950" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
      </div>
    </article>
  );
}
