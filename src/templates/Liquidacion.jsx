  ...
  return (
    <article className={`poster-page relative mx-auto w-full max-w-[794px] overflow-hidden bg-yellow-300 ${getTemplateSizeClass(tamaño)}`} style={getTemplateStyle(tamaño)}>
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-[7%] pt-[18%] text-center">
        <p className="mb-7 rounded-xl bg-black px-7 py-3 text-2xl font-black uppercase tracking-widest text-white">{mensaje || 'Todo debe irse'}</p>
        <div className="leading-none text-red-700 [text-shadow:5px_5px_0_#111827]">
        ...
        <h2 className="mt-8 max-w-[90%] break-words font-black uppercase leading-[0.9] text-black" style={{ fontSize: getProductFontSize(producto) }}>{producto || 'Nombre del producto'}</h2>
        <p className="mt-6 text-xl font-black uppercase tracking-[0.3em] text-black">{logo || 'Supermercado'}</p>
      </div>
    </article>
  );
}
